...
class iHRIS_Module_UserStatistics extends I2CE_Module {
    ...
    protected static function setupDB() {
        if ( !self::$db instanceof MDB2_Driver_Common ) {
            ...
            */
            self::$prepared['entry_history'] = self::$db->prepare("SELECT DISTINCT record,DATE(`date`) AS `date` FROM `entry` WHERE `date` >= (SELECT DISTINCT DATE(`date`) FROM entry WHERE who = ? ORDER BY `date` DESC LIMIT ?,1) AND who = ? ORDER BY `date` DESC",
    ...
    protected static function setupEntryHistory( $userid, $days = 5 ) {
        ...
        if ( array_key_exists( $userid, self::$entries ) ) {
            ...
            $result = self::$prepared['entry_history']->execute( array( $userid, $days-1, $userid ) );
